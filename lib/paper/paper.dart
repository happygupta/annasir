...
class TestScreen extends StatefulWidget {
  final int paper;
  final String time;
  TestScreen(this.paper, this.time);
  @override
...
class _TestScreenState extends State<TestScreen>
  ...
  var coco = List(150);
  int question = 0, selectedRadio, mnt, dia = 0, timerValue = 0;
  ...
  Future<void> loadFirst() async {
    ...
    if (mnt == null) {
      mnt = int.parse(widget.time);
    }
    controller = AnimationController(
      vsync: this,
      duration: Duration(minutes: mnt),
    );
    paperTime();
    start();
  ...
  Future<void> finalSubmit() async {
    if (check == false) {
      ...
      try {
        ...
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => AnalySis(widget.paper)),
        );
      } catch (e) {
        setState(() {
          correctAns = 0;
